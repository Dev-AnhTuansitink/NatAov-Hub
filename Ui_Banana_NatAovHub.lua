...
print("Tải Ui Library Thành Công !");
...
local T1UIColor = {
    ...
    ["Logo Image"] = "rbxassetid://120488231660846",
...
["Toggle Border Color"] = Color3.fromRGB(0, 120, 255),      -- Xanh dương đậm
["Toggle Checked Color"] = Color3.fromRGB(0, 170, 255),     -- Xanh dương sáng
    ["Toggle Desc Color"] = Color3.fromRGB(180, 180, 180),
    
   ["Button Color"] = Color3.fromRGB(0, 85, 170),         
...
ImageLabel.Image = "rbxassetid://120488231660846"
...
mainButton.MouseButton1Click:Connect(function()
    ...
    if isToggled then
        print("NATAOVHUB HIỆN MENU")
    else
        print("NATAOVHUB ĐÓNG MENU")
    end
end)
...
function Library:CreateWindow(Setting)
   ...
   TextLabelMain.Text = "<font color=\"rgb(0,170,255)\">" .. tostring(TitleNameMain or "Nguyen Anh Tuan") .. "</font> <font color=\"rgb(255,255,255)\">" .. tostring(getgenv().MainDesc or "") .. "</font>"
    ...
    ButtonClick()
			...
			function sectionFunction:AddButton(Setting, Callback)
				...
				ClickArea_1.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
				...
				Button_1.BackgroundColor3 = Color3.fromRGB(0, 170, 255) 
            ...
            function sectionFunction:AddDropdownSection(Setting)
               ...
               local dropdownSectionFunction = {}
				...
				function dropdownFunction:GetNewList(List)
					...
					-- [FIX] Bản gốc gán thẳng ListNew = List (mảng) cho cả dropdown Multi,
					-- nhưng refreshlist() lặp "for i, v in pairs(ListNew)" coi i là TÊN item.
					-- Với mảng thuần, i sẽ là số thứ tự (1,2,3...) → tên item hiển thị sai.
					-- Ngoài ra OrderedList (quyết định thứ tự hiển thị) không được rebuild
					-- → list trên UI không đổi dù dữ liệu đã đổi. Sửa: luôn build lại đúng
					-- cấu trúc map {tên = đã chọn?} + OrderedList mới, dù List truyền vào
					-- là mảng (["A","B"]) hay đã là map ({A=true,B=false}).
					if Selected then
						local isArray = (#List > 0) or (next(List) == nil)
						local newListNew = {}
						local newOrdered = {}
						if isArray then
							for _, name in ipairs(List) do
								newListNew[name] = false
								table.insert(newOrdered, name)
							end
						else
							for name, checked in pairs(List) do
								newListNew[name] = checked and true or false
								table.insert(newOrdered, name)
							end
						end
						ListNew = newListNew
