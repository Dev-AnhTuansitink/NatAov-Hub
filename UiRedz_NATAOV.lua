...
local natlib = {
    Themes = {
        anhtuandz = {          
            ...
            ["Color Discord Stats"] = Color3.fromRGB(0, 170, 255),
            ["Color Discord Border"] = Color3.fromRGB(0, 120, 255),
            ["Color Profile Border"] = Color3.fromRGB(0, 170, 255),
            ["Color Card Border"] = Color3.fromRGB(0, 120, 255),
            ["Color Toggle On"] = Color3.fromRGB(0, 170, 255),
            ["Color Toggle Off"] = Color3.fromRGB(20, 20, 20),
            ["Color Toggle Knob On"] = Color3.fromRGB(255, 255, 255),
            ["Color Toggle Knob Off"] = Color3.fromRGB(0, 170, 255),
            ["Color Toggle Border"] = Color3.fromRGB(0, 120, 255),
...
local Settings = natlib.Settings
local Flags = natlib.Flags

local SetProps, SetChildren, InsertTheme, Create do
    InsertTheme = function(Instance, Type)
        table.insert(natlib.Instances, {
            Instance = Instance,
            Type = Type
        })
        return Instance
    ...
    local function Save(file)
        if readfile and isfile and isfile(file) then
            ...
            if type(decode) == "table" then
                if rawget(decode, "UISize") then natlib.Save["UISize"] = decode["UISize"] end
                if rawget(decode, "TabSize") then natlib.Save["TabSize"] = decode["TabSize"] end
                if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then natlib.Save["Theme"] = decode["Theme"] end
            end
        end
    end
    
    pcall(Save, "NatAov library vip.json")
end
...
local Connections, Connection = {}, natlib.Connection do
...
local function VerifyTheme(Theme)
    for name,_ in pairs(natlib.Themes) do
        if name == Theme then
            return true
        end
    end
...
local Theme = natlib.Themes[natlib.Save.Theme]

local function AddEle(Name, Func)
    natlib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
    local Element = natlib.Elements[Ele](Instance, props, ...)
    return Element
end
...
function natlib:SetTheme(NewTheme)
    if not VerifyTheme(NewTheme) then return end
    
    natlib.Save.Theme = NewTheme
    SaveJson("NatAov library vip.json", natlib.Save)
    
    local OldTheme = Theme
    Theme = natlib.Themes[NewTheme]
    
    if MainFrame then
        if NewTheme == "anhtuandz" then
    ...
    for _, Val in pairs(natlib.Instances) do
    ...
    for _, TabData in pairs(natlib.Tabs) do
...
function natlib:SetScale(NewScale)
...
function natlib:MakeWindow(Configs)
    ...
    local UISizeX, UISizeY = unpack(natlib.Save.UISize)
    ...
    MinimizedIcon = Create("ImageLabel", MinimizedContainer, {
        ...
        AnchorPoint = Vector2.new(0, 0.5),
        BackgroundTransparency = 1,
        Image = "rbxassetid://120488231660846",
    ...
    local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
        Size = UDim2.new(0, natlib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
    ...
    ConnectSave(ControlSize1, function()
        if not Minimized then
            natlib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
            SaveJson("NatAov library vip.json", natlib.Save)
        end
    end)
    
    ConnectSave(ControlSize2, function()
        natlib.Save.TabSize = MainScroll.Size.X.Offset
        SaveJson("NatAov library vip.json", natlib.Save)
    end)
    ...
    local function UpdateSearch(Query)
        ...
        for _, ElementData in pairs(natlib.AllElements) do
    ...
    function Window:SelectTab(TabSelect)
        if type(TabSelect) == "number" then
            natlib.Tabs[TabSelect].func:Enable()
        else
            for _,Tab in pairs(natlib.Tabs) do
    ...
    function Window:MakeTab(paste, Configs)
        ...
        local function Tabs()
            ...
            table.foreach(natlib.Tabs, function(_,Tab)
        ...
        FirstTab = true
        local Tab = {}
        table.insert(natlib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
        Tab.Cont = Container
        ...
        function Tab:AddSection(Configs)
            ...
            table.insert(natlib.AllElements, {
                Name = SectionName,
            ...
            local Section = {}
            table.insert(natlib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    task.spawn(function()
        ...
        ToggleGui.Name = "NatAovHub_Toggle_Circle"
        ...
        ToggleButton.Image = "rbxassetid://120488231660846"
...
local function CreateNotification(Icon, Title, Message, Duration)
    ...
    Notification.BackgroundColor3 = Color3.fromRGB(12, 18, 28)
    ...
    IconImage.Image = Icon or "rbxassetid://120488231660846"
    ...
    TitleLabel.Text = Title or "NatAov Hub"
...
function natlib:Notify(Configs)
    local Title = Configs.Title or Configs[1] or "NatAov Hub"
    local Message = Configs.Message or Configs[2] or Configs.Text or ""
    local Icon = Configs.Icon or "rbxassetid://120488231660846"
...
function natlib:SetUIBorderColor(color)
    if typeof(color) == "Color3" then
        self.Themes[self.Save.Theme]["UI Border Color"] = color
        if self.Save.Theme == natlib.Save.Theme then
...
function natlib:SetDiscordBorderColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetProfileBorderColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetHubColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetStrokeColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetTextColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetToggleOnColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetToggleOffColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetToggleKnobOnColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetToggleKnobOffColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetToggleBorderColor(color)
    if typeof(color) == "Color3" then
...
function natlib:SetBorderThickness(thickness)
...
function natlib:GetBorderColors()
    return {
...
function natlib:GetToggleColors()
    return {
...
function natlib:ResetBorderColors()
    local defaultTheme = self.Themes.anhtuandz
...
function natlib:ResetToggleColors()
    local defaultTheme = self.Themes.anhtuandz
...
task.spawn(function()
    task.wait(2)
    natlib:Notify({
        Title = "NatAov Library",
        Message = "UI Library Load Thành Công !",
        Duration = 10
    })
end)
